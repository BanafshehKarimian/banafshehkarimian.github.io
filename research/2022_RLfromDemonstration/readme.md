...
decision process (MDP), defined by $<S, A, P, R, \gamma, \mu>$, in which
$R: S \times A \rightarrow {\rm I\!R}$ is the reward function,
$P: S \times A \times S \rightarrow [0,1]$ is the transition function,
$\gamma \in (0,1)$ is the discount factor, $\mu$ is initial state
...
follows:
$$Discrete: S = \{s_i; i\in (0, N) \} , A = \{a_i; i\in (0, M) \}$$
$$Continuous: S = <s_1,...,s_d>, s_i \in  {\rm I\!R}, A = <a_1, ..., a_q> a_i \in {\rm I\!R}$$
A policy $\pi:  S \times A \rightarrow [0,1]$ is map from a state to
...
i.e.: $$\pi^* = arg max_\pi{\rm I\!E}[\sum \gamma^t R(s_t,a_t)].$$ Our
...
loss function of GANs are as follows: $$log(D(x)+log(1-D(G(x)))$$ where
...
follows:
$$\psi \leftarrow arg \max E_{\tau \sim p*}(log(D_{\psi}(\tau))) + E_{\tau \sim \pi_{\theta}}(log(1-D_{\psi}(\tau)))$$
...
$$\pi_{\theta'} \leftarrow -A(s,a)*log(\pi_{\theta}(a|s))$$ We can use
$r(s,a,sp) = A(s,a)$ and set the reward as follows:
$$r_{\psi}(s,a,sp) = g(s) + \gamma h(sp) - h(s)$$ Where $h(s)$ is a
